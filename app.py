...
backfill.convert_to_csv("datasets/train_data.json")

# MULTIPLIER = round(len(train_data) / 10)
# BATCH_SIZE = MULTIPLIER if len(train_data) <= 320 else 32
# EPOCHS = 100

# input_data, output = backfill.setup_transposed_data(train_data)
# test_input_data, test_output_data = backfill.setup_transposed_data(test_data)
# counter = 0
