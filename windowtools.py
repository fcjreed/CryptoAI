...
def compile_learning_early_SGD(model, epoch, loss=keras.losses.Huber(),lr=1e-6, momentum=0.9,patience=10):
    compile_SGD(model, loss, lr, momentum)
    return [build_learning_rate(lr, epoch), build_early_stopping(patience)] #Return callbacks to use for fitting

def compile_early_checkpoint_SGD(model, checkpoint_name, loss=keras.losses.Huber(),lr=1e-6, momentum=0.9,patience=10):
    compile_SGD(model, loss, lr, momentum)
    return [build_early_stopping(patience), build_checkpoint(checkpoint_name)] #Return callbacks to use for fitting
...
def compile_stateful_learning_SGD(model, epoch, loss=keras.losses.Huber(),lr=1e-6, momentum=0.9,patience=10):
    compile_SGD(model, loss, lr, momentum)
